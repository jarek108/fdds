...
path = r"C:\Users\chojn\projects\gemini-cli-headless\gemini_cli_headless.py"
content = """\"\"\"
...
def _find_session_file(directory: str, session_id: str) -> str:
    \"\"\"Locates a session file matching the ID prefix. Retries with small delay to handle OS flushes.\"\"\"
...
def _execute_single_run(
    ...
    # --- SECURITY POLICY & PROMPT FILE GENERATION ---
    policy_path = None
    prompt_path = None
    ...
    try:
        ...
        if allowed_tools is not None or allowed_paths is not None:
            ...
            if policy_lines:
                with tempfile.NamedTemporaryFile(mode='w', suffix=".yaml", dir=temp_dir, delete=False, encoding='utf-8') as tf:
                    tf.write("\\n".join(policy_lines))
                    policy_path = tf.name
                # CRITICAL FIX: Append to the command list!
        ...
        if start_idx == -1 or end_idx == -1:
            raise RuntimeError(f\"CLI did not return JSON: {combined_output}\")
