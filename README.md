...
```bash
# 1. Map the Moodle platform (Optional, assumes data/documents exists)
python src/crawler.py --nodes 50

# 2. Extract knowledge into JSON traces
python src/create_document_traces.py --max-tokens 200

# 3. Compile the Master Session (using the generated traces)
...
#### 1. Crawl Platform & Map Structure
* **What it does:** Scans the FDDS Moodle platform to build a structural map of courses and pages.
* **Command:** `python src/crawler.py`

#### 2. Generate Knowledge Traces (Optional if already built)
...
#### 3. Compile Knowledge Base & Initialize Master Session
...
* **Prerequisite:** Requires JSON trace files generated by Step 2 (`create_document_traces.py`).
...
* **Prerequisite:** Requires the `data/master_session.json` generated by Step 3 (`create_master_session.py`).
